...
fn print_filenames(v: &[String], output_length: usize, width: usize) {
    ...
    if v.len() == 1 || output_length <= terminal_width {
        ...
        if terminal_width % width != 0 {
            num_columns += 1
        };

        let num_rows = v.len() / num_columns;

        if v.len() % num_columns != 0 {
            num_columns += 1
        };
...
fn print_long(v: &[String], path: &str) {
    ...
    for filename in v {
        ...
        if let Ok(metadata) = fs::symlink_metadata(file_path) {
            ...
            output.push(' ');
            output.push_str(
                get_user_by_uid(metadata.uid()).unwrap().groups().unwrap()[0]
                    .name()
                    .to_str()
                    .unwrap(),
            );
    ...
    if let Ok(entries) = fs::read_dir(&path) {
        for entry in entries {
            if let Ok(entry) = entry {
                if entry.file_name().into_string().unwrap().starts_with('.') && !flags.a_flag {
                    continue;
                } else {
